...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr
{
	...
	if ((self=[super initWithFrame:rect])) {
        ...
        for (int i=0; i<pageCount; i++) {
            ...
            EGOImageView *imgView = [[[EGOImageView alloc] initWithPlaceholderImage:[UIImage imageNamed:@""]] autorelease];

            if ([imgURL hasPrefix:@"http://"]) {
                
                [imgView performSelectorOnMainThread:@selector(setImageURL:) withObject:[NSURL URLWithString:imgURL] waitUntilDone:NO];
            }
            else
            {
            ...
            [imgView setFrame:CGRectMake(50+viewSize.size.width*i, 0,viewSize.size.width-100, viewSize.size.height)];
            imgView.tag=i;
...
//        [noteView setBackgroundColor:[UIColor colorWithRed:0.8 green:0.8 blue:0.8 alpha:0.5]];
        ...
        pageControl=[[UIPageControl alloc]initWithFrame:CGRectMake((self.frame.size.width-pageControlWidth)/2,185, pageControlWidth, pagecontrolHeight)];
