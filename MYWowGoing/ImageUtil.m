...
+ (UIImage*) blackImage:(UIImage*)startImage size:(CGSize)targetSize backgroundImage:(UIImage*)barBackImage
{
    // 有待于研究
    UIImage* backgroundImage = nil;//[UIImage tabBarBackgroundImageWithSize:startImage.size backgroundImage:barBackImage];
    
    //创建一个白色背景、黑色填充的图像
    UIImage* bwImage = nil;//[UIImage blackFilledImageWithWhitebacgroundImage:startImage];
    ...
    CGImageRef imageMask = CGImageMaskCreate(CGImageGetWidth(bwImage.CGImage),
                                             ...
                                             CGImageGetDataProvider(bwImage.CGImage),
                                             NULL,
                                             YES);
    ...
    [tabBarImage drawInRect:CGRectMake((targetSize.width/2) - (startImage.size.width/2),
                                       (targetSize.height/2) - (startImage.size.height/2),
                                       startImage.size.width, startImage.size.height) blendMode:kCGBlendModeNormal alpha:0.9];
